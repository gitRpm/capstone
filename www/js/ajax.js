...
$(document).ready(function(){
    ...
    $('.sortable').click(function(){
        ...
        $.get('taskActions.php',{
          ...
          // sort users
    $('.sortableUser').click(function(){
        var sortBy = $(this).parents('th').attr('id');
        var order;
        // arrow directions
         $('span.glyphicon-chevron-up').remove();
         $('span.glyphicon-chevron-down').remove();
         // set order param
         if ($(this).hasClass('ASC')){
             $(this).removeClass('ASC');
             $(this).append("<span class='glyphicon glyphicon-chevron-up'></span>");
         ...
         // remove arrow from other columns
         $('th span').not(this).removeClass('ASC');
        $.get('userActions.php', {
            purpose:'sort',
            sortBy:sortBy,
            order:order
...
function initialize() {
   ...
   $('[name="user-delete"]').click(function(){
       var userId = $('[name="formUserId"]').val();
       var username = $('[name="formUsername"]').val();
       if (confirm("Are you sure you want to delete " + username)) {
           $.post('userActions.php', {
               purpose:'delete',
               userId:userId
           }, function(){
               $('.table tr#'+userId).remove();
               $("#dialogEditUser").dialog("close");
               return false;
           });
       }
